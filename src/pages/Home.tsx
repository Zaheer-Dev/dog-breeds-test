...
} from "../reduxToolkit/breedService";
...
export default function Home(props: IAppProps) {
  ...
  const [getBreedsImages, { data: { message = [] } = {} }] =
    useGetBreedsImagesMutation();
  const subBreeds = useSelector(
    (state: RootState) => state?.subBreedsService.subBreeds || []
  );
  ...
  return (
    <>
      ...
      >
        ...
        <Dropdown
          nameAndLable="Breed"
          breedList={data?.message ? Object.keys(data?.message) : []}
        />
        <Dropdown nameAndLable="Sub Breed" breedList={subBreeds || []} />
        <Box sx={{ display: "flex", alignItems: "center" }}>
          <Button variant="contained" onClick={generateBreedImages}>
            Generate {subBreeds?.length || 1} Images
          </Button>
        </Box>
      </Box>
      <Dialogue dataImages={message} setOpen={setOpen} open={open} />
    </>
  );
}
